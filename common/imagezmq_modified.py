...
class ImageSender:
    ...
    def __init__(self, connect_to="tcp://127.0.0.1:5555", mode="REQ_REP"):
        """Initializes zmq socket for sending images to the hub.

        Expects an appropriate ZMQ socket at the connect_to tcp:port address:
        If mode is REQ_REP (the default), then a REQ socket is created. It
        must connect to a matching REP socket on the ImageHub().

        If mode is PUB_SUB, then a PUB socket is created. It must connect to
    ...
    def close_reqrep(self):
        """Closes reqrep connection."""
...
class ImageHub:
    ...
    def __init__(self, open_port="tcp://*:5555", mode="REQ_REP"):
        """Initializes zmq socket to receive images and text.

        Expects an appropriate ZMQ socket at the senders tcp:port address:
        If mode is REQ_REP (the default), then a REP socket is created. It
        must connect to a matching REQ socket on the ImageSender().

        If mode is SUB_PUB, then a SUB socket is created. It must connect to
...
class SerializingSocket(zmq.Socket):
    ...
    def send_array(self, arr, msg="NoName", flags=0, copy=True, track=False):
        ...
        bundle = dict(msg=msg, dtype=str(arr.dtype), shape=arr.shape,)
        self.send_json(bundle, flags | zmq.SNDMORE)
    ...
    def recv_array(self, flags=0, copy=True, track=False):
        ...
        Returns:
          msg: image name or text message.
          arr: numpy array or OpenCV image reconstructed with dtype and shape.
        """

        bundle = self.recv_json(flags=flags)
        msg = self.recv(flags=flags, copy=copy, track=track)
        arr = np.frombuffer(msg, dtype=bundle["dtype"])
        return (bundle["msg"], arr.reshape(bundle["shape"]))
    ...
    def recv_jpg(self, flags=0, copy=True, track=False):
        ...
        bundle = self.recv_json(flags=flags)  # metadata text
        jpg_buffer = self.recv(flags=flags, copy=copy, track=track)
        return (bundle["msg"], jpg_buffer)


class SerializingContext(zmq.Context):
    """Grants context to be used by zmq."""
