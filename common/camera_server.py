...
def init_sender(address, mode):
    """Initializes the ImageSender instance."""
    if not isinstance(mode, str):
        raise TypeError("Mode must be a string.")
    if not isinstance(address, str):
        raise TypeError("Adress must be a string.")
    print(f"Establishing connection {address} with REQ_REP={mode}")
    sender = imagezmq.ImageSender(connect_to=address, REQ_REP=mode)
    return sender


def user_interface_cam_ncon():
    """Handles user input of camera number and number of connections."""
...
def user_interface_add_mode(ncon, senders):
    """Adds a connection and appends it to a list (with user feedback)."""
    for con in range(ncon):
        address = str(
            input(
                f"-----------------\nAddress of {con+1}. connection\n (eg.: tcp://localhost:5555)\
                for REQ_REP\n (eg.: tcp://*:5555) for SUB_PUB\n"
            )
        )
        mode = str(input("Mode of connection (REQ_REP, PUB_SUB):"))
        if mode != "REQ_REP" or mode != "PUB_SUB":
            raise TypeError("Mode must be either REQ_REP or PUB_SUB.")
...
def add_mode_default():
    """Default mode if user doesn't input anything."""
    print(
        "Sending on address localhost:5555 with REQ_REP mode\n\
        and sending on address *:5550 with PUB_SUB mode"
    )
    senders = [
        imagezmq.ImageSender(connect_to="tcp://localhost:5555", mode="REQ_REP"),
        imagezmq.ImageSender(connect_to="tcp://*:5550", mode="PUB_SUB"),
    ]
    return senders


def run(cam):
    """Infinite loop sending images."""
    while True:
        image = cam.read()
