...
class Detection:
    ...
    def create_template(self):
        ...
        if len(rows) > 1 / but_h:
            raise ValueError(
                "There can't be more rows than can physically fit into statespace"
            )
        if len(cols) > 1 / but_w:
            raise ValueError(
                "There can't be more cols than can physically fit into statespace"
            )
        ...
        self.template = Template(
            self.buttons_raw, self.softmax_pred, self.rows_ordered, self.cols_ordered
        )
...
class Template:
    ...
    def __init__(self, buttons_raw, softmax_pred, rows_ordered, cols_ordered):
        ...
        self.priority_vh = (
            None  # True if counting by rows, false if counting by columns
        )
        self.seq = None
    ...
    def assign_template(self):
        ...
        """
        minElement = np.argmax(
            np.array(self.n_ranks)
        )  # gets the index of the best candidate
        ...
        print(
            f"The values are ordered from left to right: {self.priority_lr} \nThe values are counted by rows: {self.priority_vh}"
        )
    ...
    def count_lr(self, order):
        ...
        for row in self.rows:
            for i in row:
                listed_numbers.append(
                    self.buttons_raw[i].n_raw
                )  # get the proposed button number
    ...
    def fix_seq(self, seq):
        ...
        for i in range(len(seq) - 1):  # omit first and last button
            ...
            if seq_correct[seq_index] != True:
                ...
                if (abs(seq[seq_index - 1] - seq[valid_number_index])) == (
                    abs(valid_number_index - (seq_index - 1))
                ):
                ...
                elif (abs(seq[seq_index - 1] - seq[valid_number_index])) != (
                    abs(valid_number_index - (seq_index - 1))
                ):
